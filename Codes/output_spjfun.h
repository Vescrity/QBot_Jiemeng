  ...
  string RUA(const char*qid){
    ...
    char bf[2048];
    memset(bf,0,sizeof bf);
    string getimgurls="curl \"https://api.wer.plus/api/ruad?url=http://qlogo2.store.qq.com/qzone/";
  ...
  string hitokoto(){
    char bf[1<<13]={0};
    string toget="curl \"https://v2.alapi.cn/api/hitokoto?token="+AL_TOKEN+"\"";
  ...
  string Daily_news(){
    char bf[1<<15]={0};
    string toget="curl \"https://v2.alapi.cn/api/zaobao?format=json&token="+AL_TOKEN+"\"";
    execmd(toget.c_str(),bf);
