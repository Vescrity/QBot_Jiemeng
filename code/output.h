...
bool run_output_cmd(string msg)
{
  char bf[1 << 9] = {0};
  execmd(msg.c_str(), bf);
  ...
  else
  {
    ...
    if (get_st(word.c_str(), "blocked by server") != -1)
    {
      return 0;
    }
    return 1;
...
string message_chg2(const char *msg);
string message_chg(Msg_type type, const char *msg);
thread th_opt[128];
int thopid;
void str_output(const Msg_type &type, const char *msg, bool chg)
{
  debug_puts(msg);
  char pt[6];
  sprintf(pt, "%d", OUTPORT);
  string opt("curl -s \"");
  string smsg = message_chg2(msg);
  if (type.ifgrp)
  {
  ...
  if ((!run_output_cmd(opt)) && (!chg))
  {
    str_output(type, txt2img(msg).c_str(), 1);
  }
  msg_send_time++;
  return;
}
void output(Msg_type type, const char *msg)
{
  ...
  if (type.ord_lv > -1)
  {
    ...
    if (!main_switch || (!keep_run))
      return;
    if (smsg == "[CQ:image,file=]")
    {
      resend(type, "[NO image]");
      return;
    }
  }
  color_lable("[output]");
  ...
  if (REPORT_ID.length() > 5 || REPORT_GROUP.length() > 5)
  {
    ...
    if (rep_sid == REPORT_ID || (rep_gid == REPORT_GROUP && REPORT_GROUP.length() > 5))
      goto out_report;
    else
    {
...
out_report:
  ...
  int thid = Main_thr_num.get_new_num();
  Main_thread_list[thid] = thread(str_output, type, smsg.c_str(), false);
  if (type.msgs == "[OUTPUT_JOIN]")
  {
    Main_thread_list[thid].join();
  }
  else
  {
    Main_thread_list[thid].join();
...
void ban(Msg_type type, int t)
{
  ...
  char ttm[10] = {0};
  sprintf(ttm, "%d", t * 60);
  cmds = cmds + IP_ADD + "set_group_ban?group_id=" + type.grp_id + "&user_id=" + type.sender_id + "&duration=" + ttm + "\"";
  system(cmds.c_str());
}
void kick(Msg_type type)
{
  ...
  char pt[6];
  sprintf(pt, "%d", OUTPORT);
  cmds = cmds + IP_ADD + "set_group_kick?group_id=" + type.grp_id + "&user_id=" + type.sender_id + "\"";
  system(cmds.c_str());
}
