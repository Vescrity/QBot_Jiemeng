...
int white_list_num, Pri_white_list_num,
    ...
    msg_send_time, all_ans_num, priv_num, last_AI_time, Time_Limit;
...
string Self_ID, ANS_FILENAME, REPORT_ID, REPORT_GROUP, STAT_START_TIME,
    ...
    BAIDU_APP_ID, BAIDU_APP_SECRET, AL_TOKEN, RUN_PATH, OPEN_AI_SECRET, OPEN_AI_MODEL, TITLE;
...
bool check_priv(const char *id)
{ // 鉴权函数
  for (int i = 0; i < priv_num; i++)
  {
    if (pfct(id, priv_list[i].c_str()))
      return 1;
  }
  return 0;
}
...
void Msg_type::get_grp_name()
{
  ...
  string ord = "curl -s ";
  ord = ord + IP_ADD + "get_group_info?group_id=" + grp_id;
  execmd(ord.c_str(), bf);
  // puts(bf);
  json jbf = json::parse(bf);
  this->grp_name = jbf["data"]["group_name"];
  // this->grp_name=json_read(bf,"group_name");
}
void Msg_type::get_sender_name()
{
  char pt[6];
  sprintf(pt, "%d", OUTPORT);
  char bf[1024];
  memset(bf, 0, sizeof bf);
  string ord = "curl -s ";
  ord = ord + IP_ADD + "get_stranger_info?user_id=" + sender_id;
  execmd(ord.c_str(), bf);
  json jbf = json::parse(bf);
  this->grp_name = jbf["data"]["nickname"];
