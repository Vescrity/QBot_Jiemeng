...
bool read_config()
{
  ...
  freopen("config.cfg", "r", stdin);
  {
    ...
    if (get_st(tmptxt, "Time_Limit") == -1)
    {
      Time_Limit = 5000;
    }
    else
    {
      sscanf(tmptxt, "Time_Limit=%d", &Time_Limit);
      line_get(tmptxt);
    }
  ...
  }
  {
    ...
    info_puts(opt);
    sprintf(opt, "\rTime_Limit=%d", Time_Limit);
    info_puts(opt);
