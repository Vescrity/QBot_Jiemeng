    ...
    for (const auto &entry : fs::directory_iterator(path)) {
        if (entry.path().extension() == ".lua") {
            try {
                ...
                error_puts(string("Failed to execute ") +
...
void Lua_Shell::init(Bot *b) {
    ...
    botlib.set_function("get_answer_size", [this] {
    ...
    botlib.set_function("add_private_list", [this](const string &str) {
        this->bot->config.add_private_list(str);
    });
    botlib.set_function("add_black_list", [this](const string &str) {
        this->bot->config.add_black_list(str);
    });
    botlib.set_function("del_private_list", [this](const string &str) {
        this->bot->config.del_private_list(str);
    });
    botlib.set_function("del_group_list", [this](const string &str) {
        this->bot->config.del_group_list(str);
    });
    botlib.set_function("del_black_list", [this](const string &str) {
