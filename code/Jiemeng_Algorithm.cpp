...
std::string
Basic_String_Replace(const string &begin, const string &end,
                     const string &input,
                     std::function<std::string(const string &)> rule) {
    string result;
    ...
    if (begin.empty() && end.empty())
        throw std::invalid_argument(
            "Begin and end can't be empty at the same time");
ff:
    it = oper.find(begin);
    if (it == string::npos) {
        result += oper;
...
std::string string_format_with_json(const string &begin, const string &end,
                                    const json &js, const std::string &format) {
    return Basic_String_Replace(begin, end, format, [&js](const string &tmp) {
...
string_cut(const std::string &a, const std::string &b, const int &n) {
